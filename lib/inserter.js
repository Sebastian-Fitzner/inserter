...
Inserter.prototype = {
	...
	getFiles: function (filesOrFolders) {
		...
		filesOrFolders.forEach(function (fileOrFolder) {
			var exists = fs.existsSync(fileOrFolder);
			...
			if (exists) {
				...
				Helpers.message(
					'yellow',
					Helpers.msg.info('The folder or file ' + chalk.magenta(fileOrFolder) + ' doesn\'t exist. Nothing can be inserted :(')
				);
			}
		});
	...
	buildContent: function (content, data, match, regex) {
		...
		var newContent = keep !== 'false' ? data + '' + match : data;
	...
	checkUnused: function () {
		this.getData().forEach(function (item) {
			...
			if (!item.inserted) Helpers.message('yellow', Helpers.msg.info('For the template ' + chalk.magenta(data) + ' I haven\'t found any @ref in your insertpoints :('));
		});
	}
};
